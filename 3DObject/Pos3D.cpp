...
Vec3D Rot3D::toVector() const { return Vec3D(cos(rX) * cos(rZ), sin(rX) * cos(rZ), sin(rZ)); }
...
Pos3D Pos3D::getChildRot(radian rXRelatif, radian rZRelatif) const
{
    ...
    doubli crZR = cos(rZRelatif), crZ = cos(rZ), crX = cos(rX), srZ = sin(rZ), srX = sin(rX);
...
{
    doubli x = point.getX() + this->x, z = point.getZ() + this->z, crZ = sqrt(1 - srZ * srZ),
           crX = sqrt(1 - srX * srX), x2 = x * crZ - z * srZ;
    return Point3D(x2 * crX - (point.getY() + this->y) * srX,
                   x2 * srX + (point.getY() + this->y) * crX, x * srZ + z * crZ);
...
{
    radian rZ =
        asin((p2.getZ() - p1.getZ()) / Point3D::distance(p1, p2)); // formule getNextPos z inversée
...
{
    debug << "Pos3D(" << pos.x << "," << pos.y << "," << pos.z << ";" << pos.rX << "," << pos.rZ
          << ")";
    return debug;
}
